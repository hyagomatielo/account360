/**
 * export-solution.js — Export Account360 solution from Dynamics 365 as a managed ZIP
 */
...
async function authenticate() {
    ...
    // Try local cache first, then shared cache
    for (const cf of [CACHE_FILE, SHARED_CACHE]) {
        if (fs.existsSync(cf)) {
            pca.getTokenCache().deserialize(fs.readFileSync(cf, "utf-8"));
            break;
        }
    }
    ...
    if (accounts.length > 0) {
        try {
            const result = await pca.acquireTokenSilent({ scopes: [CRM_BASE_URL + "/.default"], account: accounts[0] });
